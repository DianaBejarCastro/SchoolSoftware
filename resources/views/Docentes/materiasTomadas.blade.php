...
<div class="container">
    ...
    <h2>LISTA DE MATERIAS TOMADAS</h2>
...
<table class="table table-bordered">
    ...
    <tbody>
        ...
        @if ($itemdata->docente_id==$datos->id)
        <tr>
            <td>{{$itemdata->nombre}}</td>
            <td>{{$itemdata->turno}}</td>
            <td>{{$itemdata->fecha_inicio}}</td>
            <td>{{$itemdata->fecha_final}}</td>
            <td><button class="btn btn-info"><a class="text-white" href="{{route('materia.destroyDocente',$itemdata->id)}}">Retirar Materia</a></button></td> 
        </tr>
        @endif
        @endforeach
    </tbody>
